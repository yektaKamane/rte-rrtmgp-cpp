...
void solve_radiation(int argc, char** argv)
{
    ...
    Array<TF,1> grid_dims({6});
    Array<TF,1> grid_x({n_col_x});
    Array<TF,1> grid_y({n_col_y});
    Array<TF,1> grid_z({n_z});
    Array<TF,1> z_lev({n_lev});
...
*/
    ...
    if (switch_shortwave)
    {
        ...
        Array_gpu<TF,3> XYZ({cam_nx, cam_ny, 3});
        ...
        //auto nc_xx = output_nc.add_variable<TF>("X", {"darn","y","x"}); 
        auto nc_xx = output_nc.add_variable<TF>("XYZ", {"n","y","x"}); 
        //auto nc_yy = output_nc.add_variable<TF>("Y", {"n","y","x"}); 
        //auto nc_zz = output_nc.add_variable<TF>("Z", {"n","y","x"}); 
